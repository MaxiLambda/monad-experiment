...
public class MListTest {
    ...
    void testTraversable() {
        ...
        var maybe = list1.traverse(new TF<Integer, Integer, Maybe<?>>(Maybe::new) {
        });

        var listlist = (MList<MList<Integer>>) list1.traverse(new TF<>((Integer v) -> new MList<>(1, v)) {
        });
    ...
    void testFoldrMZero() {
        MList<String> list1 = new MList<>("1", "2", "3");
        var res = MList.unwrap(list1.foldr(new TBF<String, MList<String>, String, MList<?>>((s, sm) -> sm.prepend(s)) {
        }));
    ...
    void filterMTest() {
        ...
        TF<Integer, Boolean, Maybe<?>> p2 = new TF<>(i -> new Maybe<>(i % 2 == 0)) {
        };
