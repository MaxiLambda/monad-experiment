...
public class ParserImpl<T> implements Parser<T> {
    ...
    public ParserImpl(Map<Symbol, OperatorToken<T>> operators) {
    ...
    @Override
    public <I> AstExpression<T> run(Lexer<I> lexer, I input) {
    ...
    public AstExpression<T> run(MList<Symbol> symbols) {
        //translate Symbols into expressions with the parsing-table from the readme
        //if a symbol is not in operators.values(), L_BRACE.getSymbol() or R_BRACE.getSymbol() it has to be a Literal
        return generateAst(INITIAL_PRODUCTION.apply(operatorStrength::get), new ArrayDeque<>(symbols.toList()));
    }
